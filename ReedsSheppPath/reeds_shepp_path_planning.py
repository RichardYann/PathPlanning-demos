...
class Path:
    ...
    def __init__(self):
        # course segment length 路线片段长度  (negative value is backward segment)
        self.lengths = []
        # course segment type char ("S": straight, "L": left, "R": right) 每一个segment方向一致
        self.ctypes = []
...
def plot_arrow(x, y, yaw, length=1.0, width=0.5, fc="r", ec="k"):
    if isinstance(x, list):          #判断是否列表类型
        for (ix, iy, iyaw) in zip(x, y, yaw):
            plot_arrow(ix, iy, iyaw)
    else:
        plt.arrow(x, y, length * math.cos(yaw), length * math.sin(yaw), fc=fc,  #不是列表类型则（x，y）为起点坐标（这种可能是画直线） head箭头
...
def mod2pi(x):
    ...
    v = np.mod(x, np.copysign(2.0 * math.pi, x))  #np.copysign将第一个数的符号转换成第二个数的符号 mod求余
    if v < -math.pi:
        v += 2.0 * math.pi
    else:
        if v > math.pi:
...
def set_path(paths, lengths, ctypes, step_size):
    path = Path()
    path.ctypes = ctypes
    path.lengths = lengths  #每一段的长度（带方向）
...
def straight_curve_straight(x, y, phi, paths, step_size):  #  paths = straight_curve_straight(x, y, dth, paths, step_size)
