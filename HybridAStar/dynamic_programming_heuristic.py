...
def calc_final_path(goal_node, closed_node_set, resolution):
    ...
    rx, ry = [goal_node.x * resolution], [goal_node.y * resolution]  #不用加min_x，是因为起点在原点？
...
def calc_distance_heuristic(gx, gy, ox, oy, resolution, rr):
    ...
    ox = [iox / resolution for iox in ox]  #index
    ...
    priority_queue = [(0, calc_index(goal_node, x_w, min_x, min_y))] # cost index
